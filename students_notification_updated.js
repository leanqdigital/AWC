    ...
    query calcContacts($id: AwcContactID) {
        calcContacts(query: [{ where: { id: $id } }]) {
            ...
            Turn_Off_All_Notifications_Time_Unix: field(arg: ["turn__off__all__notifications__time__unix"])
...
`;
    ...
    async function buildDynamicNotificationQuery(preferences) {
        const conditions = [];
    ...
    `);
        ...
        if (!preferences.Post_Comments || !preferences.Comments_On_My_Posts) {
            conditions.push(`
        {
            orWhereGroup: [
                { where: { notification__type: "Post Comments" } },
                {
                    andWhere: {
                        Comment: [{ where: { author_id: $author_id, _OPERATOR_: neq } }]
                    }
                }
            ]
        }
    `);
        ...
        {
            orWhereGroup: [
                { where: { notification__type: "Announcements" } },
                { andWhere: { instructor_id: $instructor_id, _OPERATOR_: neq } }
            ]
        }
    `);
        }

        if (!preferences.Announcement_Comments || !preferences.Comments_On_My_Announcements) {
            conditions.push(`
        {
            orWhereGroup: [
                { where: { notification__type: "Announcement Comments" } },
                {
                    andWhere: {
                        ForumComments: [{ where: { author_id: $author_id, _OPERATOR_: neq } }]
                    }
                }
            ]
        }
                ...
                { where: { notification__type: "Submissions" } },
                {
                    orWhere: {
                        Submissions: [
                            {
                                where: {
                                    Student: [{ where: { student_id: $student_id, _OPERATOR_: neq } }]
                                }
                            }
                        ]
                    }
    ...
    `);
        ...
        if (!preferences.Submission_Comments || !preferences.Comments_On_My_Submissions) {
            conditions.push(`
        {
            orWhereGroup: [
                { where: { notification__type: "Submission Comments" } },
                {
                    orWhere: {
                        Submissions: [
                            {
    ...
    `);
        ...
        if (!preferences.Post_Mentions) {
            conditions.push(`
        {
            orWhereGroup: [
                { where: { notification__type: "Post Mentions" } },
                {
                    orWhere: {
                        Post: [{ where: { Mentions: [{ where: { id: $id } }] } }]
                    }
                }
            ]
        }
    `);
        ...
        if (!preferences.Post_Comment_Mentions) {
            conditions.push(`
        {
            orWhereGroup: [
                { where: { notification__type: "Post Comment Mentions" } },
                {
                    orWhere: {
                        Post: [{
                            where: {
                                ForumComments: [{
                                    where: { Mentions: [{ where: { id: $id } }] }
                                }]
                            }
                        }]
                    }
    ...
    `);
        ...
        if (!preferences.Submission_Comment_Mentions) {
            conditions.push(`
        {
            orWhereGroup: [
                { where: { notification__type: "Submission Comment Mentions" } },
                {
                    orWhere: {
                        Submissions: [{
                            where: {
    ...
    `);
        ...
        if (!preferences.Announcement_Mentions) {
            conditions.push(`
        {
            orWhereGroup: [
                { where: { notification__type: "Announcement Mentions" } },
                { orWhere: { Mentions: [{ where: { id: $id } }] } }
            ]
        }
    `);
        }

        if (!preferences.Announcement_Comment_Mentions) {
            conditions.push(`
        {
            orWhereGroup: [
                { where: { notification__type: "Announcement Comment Mentions" } },
                {
                    orWhere: {
                        ForumComments: [{ where: { Mentions: [{ where: { id: $id } }] } }]
                    }
                }
            ]
        }
    `);
        ...
        const createdAtCondition = (!preferences.Turn_Off_All_Notifications && preferences.Turn_Off_All_Notifications_Time_Unix)
            ? `{ andWhere: { created_at: ${preferences.Turn_Off_All_Notifications_Time_Unix} } },`
            : "";
...
`;
    ...
    calcEnrolments(
        query: [
        { where: { student_id: [Visitor//Contact ID] } }
        {
            andWhereGroup: [
...
`;
    ...
    async function fetchClassIds() {
        const prefVariables = { id: LOGGED_IN_CONTACT_ID };
        try {
            const prefResponse = await fetch(HTTP_ENDPOINT, {
                method: "POST",
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    variables: prefVariables,
                }),
            });

            const prefResult = await prefResponse.json();
            const preferences = prefResult?.data?.calcContacts?.[0];

            if (!preferences || preferences.Turn_Off_All_Notifications) return [];

            const dynamicQuery = await buildDynamicNotificationQuery(preferences);
            if (!dynamicQuery) return [];

            FETCH_NOTIFICATIONS_SUBSCRIPTION_QUERY = dynamicQuery;
            ...
            const response = await fetch(HTTP_ENDPOINT, {
                ...
                body: JSON.stringify({
                    query: FETCH_CLASSES_QUERY
                }),
            });

            const result = await response.json();
            if (result.data && result.data.calcEnrolments) {
                return result.data.calcEnrolments.map(cls => cls.Class_ID);
            }
            return [];
        } catch (error) {
            return [];
...
<!-- 
If type is post mention

1) Post author display name
2) Post parent course/class
3) Mention users data

Title:  Post author display name mentioned you in a post
Content: You have been mentioned by Post author display name in a post - Course/Class Name 
-->
